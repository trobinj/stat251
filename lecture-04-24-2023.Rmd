...
tbl <- add_header_row(tbl, colwidths = c(1, 2, 1),
  values = c("","Strep",""))
...
tbl <- flextable(d) %>% set_table_properties(layout = "autofit")
tbl <- add_header_row(tbl, colwidths = c(1, 4, 1),
  values = c("","Tumor Response",""))
tbl <- align(tbl, part = "body", align = "center")
tbl <- align(tbl, part = "header", align = "center")
tbl
```
...
tbl <- flextable(head(d, 10))
tbl <- set_table_properties(tbl, layout = "autofit")
tbl
```
...
d <- data.frame(Patient = c("tonsillectomy","no tonsillectomy"),
  yes = c("p~a~","p~c~"), no = c("p~b~","p~d~"))
names(d) <- c("Patient (Case)", "tonsillectomy", "no tonsillectomy")
tbl <- flextable(d) %>%
  set_table_properties(layout = "autofit") %>% 
  align(part = "body", align = "center") %>% 
  align(part = "header", align = "center") %>% colformat_md()
tbl
```
...
$$
  \hat{p}_b = \frac{7/85+15/85}{2}, \ \ \hat{p}_c = \frac{7/85+15/85}{2}.
$$
...
$$
  n \times \hat{p}_b = 85 \times \frac{7/85+15/85}{2} = \frac{7+15}{2} = 11, \\ \\ 
  n \times \hat{p}_c = 85 \times \frac{7/85+15/85}{2} = \frac{7+15}{2} = 11.
$$
