...
p <- p + geom_hline(yintercept = 0.8, color = grey(0.5), linewidth = 0.5)
p <- p + geom_point(size = 1)
p <- p + geom_linerange(aes(ymin = lowb, ymax = uppb), linewidth = 0.25)
p <- p + xlab("Sample")
...
$$
Approximately how many confidence intervals would contain $p$? What if we replaced 1.96 with 2.576?
...
p <- p + geom_hline(yintercept = mu, color = grey(0.5), linewidth = 0.5)
p <- p + geom_point(size = 1)
p <- p + geom_linerange(aes(ymin = lowb, ymax = uppb), linewidth = 0.25)
p <- p + xlab("Sample")
