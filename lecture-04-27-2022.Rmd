...
ktbl(d, align = "lccc") %>% add_header_above(c(" " = 1, "Outcome" = 2, " " = 1))
```
...
ktbl(d, align = "llccc") %>% add_header_above(c(" " = 2, "Outcome" = 2, " " = 1))
```
Why?
...
names(d)[4] <- "Total"
ktbl(d, align = "lccc") %>% group_rows("White Victim", 3, 4) %>% group_rows("Black Victim", 5, 6) %>%
  add_header_above(c(" " = 1, "Death Penalty" = 2, " " = 1))
```
...
ktbl(d) %>% add_header_above(c(" " = 1, "Cholecystitis" = 2, " " = 1)) %>% column_spec(1, bold = TRUE)
```
...
d <- data.frame(Helmet = c("yes","no","Total"),
  ...
  no =  c(paste(x[1,2], " (", round(x[1,2]/sum(x[1,]), 2), ")", sep = ""), 
         paste(x[2,2], " (", round(x[2,2]/sum(x[2,]), 2), ")", sep = ""), sum(x[,1])),
  Total = c(sum(x[1,]), sum(x[2,]), sum(x)))
ktbl(d) %>% add_header_above(c(" " = 1, "Hospitalized" = 2, " " = 1)) %>% column_spec(1, bold = TRUE)
```
...
ktbl(d, align = "cccc") %>% add_header_above(c(" " = 1, "Success" = 2, " " = 1))
```
...
ktbl(d, align = "ccccc") %>% add_header_above(c(" " = 2, "Success" = 2, " " = 1))
```
