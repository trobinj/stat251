...
With two **dependent samples**, the observations in the two samples are somehow "linked" between the samples by design (e.g., same unit observed twice, twins). Matched-pairs designs create dependent samples.
...
With two **independent samples**, the observations in the two samples are *not* "linked" between the samples.
...
<!-- Note: The sample sizes are backwards here but I left them as is since we did the example already. But the aspirin group has the larger n. Be sure to fix ALL the tables in the next lecture. -->
...
```{r}
d <- data.frame(Group = c("aspirin","control"), yes = c(104,189), no = c(11034-104,11037-189), Total = c(11034,11037))
