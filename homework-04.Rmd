...
The figure below shows what is sometimes called a *triangular distribution*. 
```{r, fig.height = 4}
library(ggplot2)
...
{
  b <- 2/y
  if (b * y / 2 > 1 | b < x | x < 0 | y < 0) {
    warning("improper PDF")
  }
  
  d <- data.frame(x = c(0, x, b), y = c(0, y, 0))
  
  p <- ggplot(d, aes(x = x, y = y)) + theme_minimal() + 
    geom_line() + labs(x = "x", y = "Probability Density")

  return(p)
}


p <- f(1, 0.5) + scale_x_continuous(breaks = seq(0, 4, by = 0.5)) +
  scale_y_continuous(breaks = seq(0, 0.5, by = 0.05))
plot(p)
```
Use this figure to confirm the following probabilities. (Hint: The area of a right triangle is base times height divided by two.)

1. The probability of $x < 1$ is 0.25.

0. The probability of $x > 1$ is 0.75.

0. The probability of $x < 0.5$ is 0.0625.

0. The probability of $x > 2.5$ is 0.1875.

0. The probability of $0.5 < x < 2.5$ is 0.75.
