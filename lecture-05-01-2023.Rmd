...
r <- data.frame(r) %>% rownames_to_column("Variable")

tbl <- flextable(r) %>% set_table_properties(layout = "autofit")
tbl <- align(tbl, part = "all", align = "center")
tbl <- align(tbl, j = 1, align = "left")
tbl <- add_header_row(tbl, colwidths = c(1,9), values = c("","Variable"))
tbl
```
